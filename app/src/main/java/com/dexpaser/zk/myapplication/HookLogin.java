...
public class HookLogin {
	...
	public static void HookImp(final XC_LoadPackage.LoadPackageParam lpparam)
	{
		...
		{
		Class<?> class1 = XposedHelpers.findClass("android.view.View",lpparam.classLoader);
		 try {
			 Class<?> aClass = XposedHelpers.findClass("android.support.v4.app.FragmentActivity", lpparam.classLoader);

			 if (aClass==null){
				 return;
			 }

			 XposedHelpers.findAndHookMethod("android.support.v4.app.FragmentActivity", lpparam.classLoader,
					"viewToString", class1, new XC_MethodHook() {
				@Override
